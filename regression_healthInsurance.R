...
nRows <- nrow(train)
nRows
# Implement the 10-fold cross-fold plan with vtreat
# set.seed(562)
set.seed(562)
...
# get the R^2 from train data of CV
(rsq.cv <- r_squared(train$charges, train$pred.cv))
...
##Ridge regression model by 10-folds (default) cross-validation 
set.seed(562)
...
# Plot the predictions (x-axis) against the outcome (charges) for linear models about test data
...
##As we can see that in the plots of predictions against the real insurance charges, the points displayed in partitions. Especially for higher charges, the points showed binary distributions beside the y=x line, which indicates that there likely to be some interactions between variables. So next we do some exploratory analysis by coloring some variables in the plot to check out the significant categorical factors.   
...
train.mat<-model.matrix(formula(~ (age+sex+bmi+children+smoker+region)^2+I(age^2)+I(bmi^2)+I(children^2)),data=train)
test.mat<-model.matrix(formula(~ (age+sex+bmi+children+smoker+region)^2+I(age^2)+I(bmi^2)+I(children^2)),data=test)
set.seed(562)
